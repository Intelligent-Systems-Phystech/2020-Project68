...
class AGNews:
    ...
    def __init__(self, data_folder, ngrammer=None, lemmatize=True, exclude_stop_words=True, exclude_label_field=True):
        ...
        self.test_labels = []
        self.name = "AG's news"
        dataset_name = self.name.lower().replace("'", '').replace(' ', '_')
        if not lemmatize:
            path = os.path.join(data_folder, f'{dataset_name}/original_text/')
        elif not exclude_stop_words:
            path = os.path.join(data_folder, f'{dataset_name}/lemmatized/')
        else:
            path = os.path.join(data_folder, f'{dataset_name}/lemmatized_wo_stopwords/')
